...
public class M_file {
	...
	public boolean cdn_filesave(file_DTO f_dto, MultipartFile image_file, String url) throws IOException {
		...
		try {
				...
				if(this.fc.login(this.user, this.pass)) {  //ftp로그인
					...
					this.result = this.fc.storeFile("/이미지저장폴더명쓰기/"+this.file_rnm, image_file.getInputStream());  //ftp디렉토리 경로 설정 후 해당 파일을 byte로 전송
					//파일 먼저 저장 후 DB저장
					...
					String api_url = "http://"+url+"/이미지저장폴더명쓰기/"+this.file_rnm;
					f_dto.setImgPath(api_url);  //이미지경로
					
					File dir = new File("http://"+url+"/이미지저장폴더명쓰기/");		
...
//			String filesize = this.fc.getSize("/nyong/"+fileyes);
//			if(filesize != null) {  //ftp에 파일이 있을 때 
//				this.result = this.fc.deleteFile("/nyong/"+fileyes);  //FTP에 있는 해당 파일명의 파일 삭제 
