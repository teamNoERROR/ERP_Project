...
public class mrp_Calulation {
	...
	public List<mrp_result_DTO> mrp_calc(List<mrp_input_DTO> mrp_inputs) {
		...
		for (mrp_input_DTO input : mrp_inputs) {
			...
			for (temp_bom_DTO bom : boms) {
				...
				aggregated.merge(item_code,
						new mrp_result_DTO(item_code, bom.getItem_name(), bom.getItem_type(), required_qty, bom.getItem_unit(), total_stock, safety_stock, reserved_stock, available_stock, shortage_stock),
                        (oldVal, newVal) -> {
		...
		System.out.println(new ArrayList<>(aggregated.values()));
